...
TimeType TimeType::operator+(int hrs) const
{
	...
	return TimeType(hrs, m_mins, m_secs);
}

TimeType operator+(int hrs, const TimeType& time)
{

	hrs += time.m_hrs;   // How does it access this private variable?
	if (hrs > 12)
	{
		hrs %= 12;
	}

	return TimeType(hrs, time.m_mins, time.m_secs);  // Here too?
}
